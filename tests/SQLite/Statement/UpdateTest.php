...
namespace FaaPz\PDO\QueryBuilder\Tests\SQLite\Statement;
...
{
    /** @var SQLite\Statement\Update $subject */
    private $subject;
    ...
    public function setUp(): void
    {
        ...
        $this->subject = new SQLite\Statement\Update($pdo);
    }
    ...
    public function testToStringWithRaw()
    {
        $this->subject
            ->table('test')
            ->set('col', new SQLite\Clause\Raw('col + 1'));
    ...
    public function testToStringWithJoin()
    {
        $this->subject
            ->table('test1')
            ->set('col', 'value')
            ->join(new SQLite\Clause\Join(
                'test2',
                new SQLite\Clause\Conditional('test1.id', '=', 'test2.id')
            ));
    ...
    public function testToStringWithWhere()
    {
        $this->subject
            ->table('test')
            ->set('col', 'value')
            ->where(new SQLite\Clause\Conditional('id', '=', 1));
    ...
    public function testGetValuesWithWhere()
    {
        $this->subject
            ->table('test')
            ->set('col', 'value')
            ->where(new SQLite\Clause\Conditional('col', '<>', 5));
